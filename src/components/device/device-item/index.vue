<template>
  <div
    class="device-item text-size-sm text-666 shadow margin-x-2 rounded-md overflow-hidden"
    :class="{ active: !deviceBindOwn }"
  >
    <hd-card class="padding-2 bg-white">
      ...
      >
        <span>端口状态：</span>
        <span>
          <van-tag type="success" round class="margin-right-1"
            >空闲 ({{ value.freenum }})</van-tag
          >
          <van-tag type="danger" round class="margin-right-1"
            >占用 ({{ value.usenum }})
          </van-tag>
          <van-tag type="warning" round class="margin-right-1"
            >故障 ({{ value.failnum }})</van-tag
          >
        </span>
      </hd-card-item>
      <div class="device-contral d-flex justify-content-end w-100 margin-top-1">
        ...
        <van-button
          ...
          size="small"
          v-if="
            !['03', '04', '11'].includes(value.hardversion) &&
              value.device_type !== 2 &&
              deviceBindOwn
          "
        ...
        >
        <van-button
          :type="value.state === 1 ? 'primary' : 'danger'"
          size="small"
          v-if="value.device_type !== 2"
        >
          <img
            class="singal-icon"
            :src="require(`../../../assets/images/singal/${singalIcon}`)"
          />
        </van-button>
        <van-button
          type="primary"
