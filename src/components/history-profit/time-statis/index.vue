<template>
    <div class="time-statis">
        <div class="time-top padding-x-3 padding-y-2">
            <h3>营收总额（元）</h3>
            <div class="text-success font-weight-bold math-num total-money">{{ gather.total | fmtMoney }}</div>
            <hd-card>
                <hd-card-item class="d-flex" v-for="item in gather.list" :key="item.title">
                    ...
                    >
                        ...
                        <template #reference>
                            <div>
                                <van-icon class="margin-left-1" name="question-o" size=".4rem" />
                            </div>
                        </template>
                    </van-popover>
                </hd-card-item>
