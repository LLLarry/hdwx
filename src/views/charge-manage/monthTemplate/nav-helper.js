...
import { amendTempMonth } from '@/require/template'
...
export default ({ type, isSystemTem, tempData, copyTempData }, router, code) => {
    ...
    // 保存模板入口
    const saveTemp = () => {
        amendTempMonth({ paratem: JSON.stringify(tempData.value) })
        .then(res => {
            if (res.code === 200) {
                alert('模板保存成功')
                .then(() => {
    ...
    const goBack = () => {
        ...
        const result = Object.entries(copyTempDataObj).every(([key, value]) => {
            if (['tempname', 'daymaxnum', 'everymaxtime', 'everymaxelec', 'monthmaxnum', 'month'].includes(key)) {
                value = value === null ? '' : value
                tempDataObj[key] = tempDataObj[key] === null ? '' : tempDataObj[key]
                return value.toString() === tempDataObj[key].toString()
            } else if (key === 'tempson') {
                return (value || []).every(({ sonname, paymoney, monthnum }, index) => {
                    const { sonname: oldSonname, paymoney: oldPaymoney, monthnum: oldMonthnum } = tempDataObj[key][index]
                    return paymoney.toString() === oldPaymoney.toString() && sonname.toString() === oldSonname.toString() && monthnum.toString() === oldMonthnum.toString()
                })
            }
            return true
        })
