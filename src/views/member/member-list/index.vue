<template>
  <div class="member-list position-relative d-flex flex-column">
    <div class="section shadow padding-bottom-2">
      <div
        class="search-form d-flex padding-x-2 margin-top-2 align-items-center"
      >
        <van-dropdown-menu class="select-menu">
          <van-dropdown-item
            get-container=".dropMenu"
            v-model="type"
            :options="option1"
          />
        </van-dropdown-menu>
      ...
      <div class="padding-top-2">
        <van-dropdown-menu class="select-menu">
          <van-dropdown-item
            ...
            v-model="value2"
            :options="option2"
            @change="asyAjaxMemberCentre(true)"
          />
          <van-dropdown-item
            get-container=".dropMenu"
            v-model="value3"
      ...
      <div
        class="padding-top-2 padding-x-2 d-flex justify-content-around text-size-sm"
      >
        <div>
          <span>数量：</span><span class="text-success">{{ datasize }}个</span>
        </div>
        <div>
          <span>充值：</span
          ><span class="text-success">{{ datatopupmoney }}元</span>
        </div>
        <div>
          <span>赠送：</span
          ><span class="text-success">{{ datasendmoney }}元</span>
        </div>
      </div>
    </div>
    ...
    <main class="bg-gray">
      <hd-scroll @pullingUpFn="pullingUpFn" @getScroll="getScroll">
        ...
        <div class="padding-top-3">
          ...
          />
          <div class="text-center padding-bottom-3 text-666">
            {{ status === 2 ? '暂无更多数据' : '正在加载更多' }}
          </div>
        </div>
      </hd-scroll>
    </main>
  ...
  computed: {
    descMessage() {
      if (this.actionIsShow) {
        ...
        return `更改${nick}（${this.actionRow.uid
          .toString()
          .padStart(8, 0)}）用户的归属小区`
      }
      return ''
    }
  },
    ...
    async asyAjaxMemberCentre(init = false) {
      try {
        ...
        if (init) {
          ...
          this.option2 = [
            { text: '所有小区', value: '' },
            ...areaData.map(({ name, id }) => ({ text: name, value: id }))
          ]
          this.actions = [
            { name: '不绑定小区', id: 0 },
            ...areaData.map(({ name, id }) => ({ name, id }))
          ]
        } else {
    ...
    async handldConfirmChnageArea({ id, name, subname }) {
      if (getType(subname) === 'undefined') {
        try {
          ...
          if (code === 200) {
            // 查询到会员信息
            const member = this.list.find(
              item =>
                item.uid === this.actionRow.uid &&
                item.aid === this.actionRow.aid
            )
