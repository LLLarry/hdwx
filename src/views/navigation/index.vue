<template>
  <div class="navigation-page">
    <header class="bg-gray">
      <van-search v-model="searchValue" placeholder="请输入导航关键词" readonly @click="slideSearch= true" />
    </header>
    <div class="post-menu d-flex">
      <div class="slide flex-1 bg-gray">
        <ul>
          <li class="padding-y-1" v-for="(item, index) in map" :key="item.title" @click="handleSelect(index)" :class="{ 'bg-white': active === index }">
            <div class="text-center text-999 padding-y-2" :class="{ 'text-success active font-weight-bold': active === index }">{{item.title}}</div>
          </li>
        </ul>
      </div>
          ...
          <div class="wrapper">
            <div class="block overflow-hidden" v-for="item in map" :key="item.title">
              ...
              <ul class="d-flex flex-wrap">
                <li class="w-50 text-center padding-y-3" v-for="one in item.list" :key="one.name" @click="handleRouter(one)">
            ...
            <div class="bottom-race" :style="{ height: `${postMenuHeight * .8}px` }"></div>
          </div>
        </hd-scroll>
      </div>
    </div>
    <!-- 搜索列表 -->
    <van-popup v-model="slideSearch" position="right" :style="{ height: '100%', width: '100%' }">
       <section class="bg-gray d-flex flex-column">
          <div class="d-flex align-items-center bg-white">
            <van-search v-model="searchValue" class="flex-1" placeholder="请输入导航关键词" />
            <span class="padding-right-3 text-success padding-y-3" @click="slideSearch = false">取消</span>
          </div>
          <div class="flex-1 padding-3">
            <ul class="search-post bg-white padding-2 rounded-md" v-if="filterList.length > 0">
              <li class="padding-y-1 d-flex align-items-center" v-for="item in filterList" :key="item.name" @click="handleRouter(item)">
...
const map = [
  ...
  },
  {
    title: '提现',
    list: [
      { name: '提现到微信', url: '/withdraw/page/3', icon: require('@/assets/images/mine/提现.png'), class: 'sm-img' },
      { name: '提现到银行卡', url: '/withdraw/page/1', icon: require('@/assets/images/mine/提现.png'), class: 'sm-img' },
      { name: '提现到对公', url: '/withdraw/page/2', icon: require('@/assets/images/mine/提现.png') },
      { name: '银行卡管理', url: '/withdraw/mybankcard', icon: require('@/assets/images/mine/卡片.png'), class: 'sm-img' },
...
export default {
  ...
  methods: {
      ...
      fmtSearch (value) {
        ...
        const result = [...fmtSecondList1, ...fmtSecondList2].reduce((acc, item) => {
