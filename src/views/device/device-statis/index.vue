           ...
           <van-sticky offset-top="1.3866666666666667rem" v-if="showincoins">
            ...
            <hd-scroll @pullingUpFn = "pullingUpFn" @getScroll="({ scroll }) => this.scroll = scroll">
                <div class="padding-y-3">
                    <van-row class="d-flex post-border-row margin-x-3 text-size-sm text-666" v-for="item in list" :key="item.id">
...
import { inquireDeviceIncomeInfo } from '@/require/device'
const LIMIT = 30
export default {
    data () {
        return {
    ...
    computed: {
      spanNum () {
        ...
        if (this.hardversion !== '03' && this.hardversion !== '04') {
          num++
        }
        return 24 / num
      }
    ...
    methods: {
        async gatDeviceStatis (init = false) {
            if (init) {
               this.currentPage = 1
            } else {
                ++this.currentPage
            ...
            try {
                this.status = 0
                const { code, message, ...result } = await inquireDeviceIncomeInfo({
