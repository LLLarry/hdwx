          ...
          >
              ...
              >
                <div class="">
                  <div class="font-weight-bold text-666 text-size-md card-num ">
                    交易金额：<span class="text-666"
                      >&yen; {{ item.paymoney | fmtMoney }}</span
                    >
                  </div>
                </div>
              </div>
            ...
            <hd-card class="padding-2 text-size-sm">
              ...
              <hd-card-item class="">
                ...
                <span
                  class="card-item-content text-success"
                  v-if="item.number === 0"
                  >正常</span
                >
                <span
                  class="card-item-content text-danger"
                  v-else-if="item.number === 1"
                  >全额退款</span
                >
                <span
                  class="card-item-content text-warning"
                  v-if="item.number === 2"
                  >部分退款</span
                >
              </hd-card-item>
              ...
              <hd-card-item class="" v-if="!is0304">
...
export default {
  ...
  methods: {
    ...
    async handleRefund({ id, paytype, ordersource }) {
      this.$dialog
        .confirm({
          title: '提示',
          message: '确认退费吗？'
        ...
        .then(() => {
          this.refundFn(id, paytype, ordersource)
        })
    },
    // 退费函数
    async refundFn(id, paytype, ordersource) {
      const refundState = ordersource // 1:充电、 2:离线、 3:投币
      let payTypeNum // 支付类型
      let wolfkey = 0
      switch (paytype) {
        case 1:
  ...
  computed: {
    is0304() {
      return ['03', '04'].includes(this.hardversion)
    }
  }
}
</script>
